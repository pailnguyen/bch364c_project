...
We stored the fluorescent microscopy images in this same format.
The localization class is represented by 1 byte (as values range from 0 to 10).
This byte is then followed by a series of bytes representing luminance
(1 byte per pixel).
...
def read_jkl(filename_queue):
  ...
  # transpose: [depth, height, width] to [height, width, depth].
...
def distorted_inputs(data_dir, batch_size):
  ...
  read_input = read_jkl(filename_queue)
  ...
  # Randomly flip vertically as well.
  distorted_image = tf.image.random_flip_up_down(distorted_image)
  # Rotate by 90 degrees between 0 and 3 times (inclusive).
  distorted_image = tf.image.rot90(distorted_image, k=random.randint(0, 3))
